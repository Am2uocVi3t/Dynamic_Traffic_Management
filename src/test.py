...
def find_reference_image(flow_type, time_of_day, lane, input_image):
    ...
    if best_ref_image:
        # print_success(f"Best Image for {lane} in {flow_type} ({time_of_day}): {best_ref_image} | Score: {best_score}")
...
def determine_traffic_level(input_image, time_of_day):
    ...
    for level in ["low", "mid", "high"]:
        ref_left_path = find_reference_image(f"{level}_flow", time_of_day, "left", left_image)
        ref_right_path = find_reference_image(f"{level}_flow", time_of_day, "right", right_image)

        if ref_left_path:
            ref_left_image = Image.open(ref_left_path).convert("L")
            scores["left"][level] = compare_images(left_image, ref_left_image)

        if ref_right_path:
            ref_right_image = Image.open(ref_right_path).convert("L")
